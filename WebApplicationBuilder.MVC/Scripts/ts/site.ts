...
//  */
function dropZoneEventsAdder(item:any){
    ...
    item.addEventListener('drop',(event:any)=>{
        ...
        const toolType:string = draggingElement.getAttribute('toolType');
        if(toolType=="textBox")
        {
            console.log("textBox");
            const t:TextInputRepository = new TextInputRepository();
...
draggableFields.forEach((item:any)=>{
...
function autoGridRowAdapter()
{
    ...
    const lastRow:any = document.getElementById('output-grid-table-body')?.lastElementChild;
    Array.from(lastRow.children).forEach((item:any) => {
        console.log(item);
...
//Setting
const table_grid_checkbox:any= document.getElementById('show-table-grid-checkbox');
...
function changeGridRowNumber(numberOfColumn:any){
    ...
    allTds.forEach((td)=>{
        ...
        const number_of_columns_1:any =  document.getElementById('number_of_columns_1');
        const number_of_columns_2:any =  document.getElementById('number_of_columns_2');
        const number_of_columns_3:any =  document.getElementById('number_of_columns_3');
        if(numberOfColumn==1)
        {
    ...
    gridRows.forEach((row)=>{
        ...
        let columns:any = numberOfColumn;
        let currentColumns:any=0;
        ...
        let last_td:any = row.lastElementChild;
        while(currentColumns>columns)
        {
...
const number_of_column_grid_form:any = document.getElementById('number_of_column_grid_form');
number_of_column_grid_form.addEventListener('change',(event:any)=>{
...
function toggleGridLine(status:any){
    const newSettings:any = JSON.parse(localStorage.getItem('userSetting'));
    const table_grid_checkbox:any= document.getElementById('show-table-grid-checkbox');
    const ouputGridTable:any = document.getElementById('output-grid-table');
    if(status)
    {
